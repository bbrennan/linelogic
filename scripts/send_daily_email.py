...
def build_daily_email(today: str, predictions: list[dict]) -> str:
            ...
            html += f"""
              <tr style=\"border-bottom: 1px solid #eee;\">
                ...
                <td style=\"padding: 12px; text-align: center;\">
                  {pred.get('home_team_rest_days', '-')}d
                </td>
                <td
                  style=\"padding: 12px;
    ...
    html += """
          ...
          >
            ...
            🟢 <strong>TIER 1:</strong> Use model predictions directly
            (≥70% expected accuracy)<br/>
            🟡 <strong>TIER 2:</strong> Use with external validation
            (68-70%)<br/>
            🔴 <strong>TIER 4:</strong> Cross-check externally
            - back-to-back weakness<br/>
          </div>
          <p
            style="margin-top: 20px;
                   font-size: 12px;
...
def send_email(today: str) -> None:
    candidate_paths = [
        f"docs/status/daily/predictions_{today}.csv",
        f"predictions_{today}.csv",  # legacy fallback
    ]
    ...
    if not predictions:
        print("⚠️  Predictions CSV not found. Skipping email.")
        return
